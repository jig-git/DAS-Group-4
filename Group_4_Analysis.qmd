---
title: "AN In-Depth Look at BMI in Scotland from 2008-2012"
...
bmi_summary <- DAProject8 %>%
  ...
  cols_width(
    everything() ~ px(60)  # Narrow fixed width for all columns (60 pixels)
  ) %>%
  tab_options(
    table.font.size = 9,            # Further reduced from 10
    heading.title.font.size = 12,   # Reduced from 14
    heading.subtitle.font.size = 10, # Reduced from 12
    table.width = pct(80),          # Kept at 80% page width
    column_labels.padding = px(2),  # Tighten column label padding
    data_row.padding = px(2)        # Tighten row padding
  )
bmi_summary
```
...
bmi_by_year <- DAProject8 %>%
  ...
  cols_width(
    Year ~ px(50),          # Narrower for Year (text)
    Mean_BMI ~ px(60),      # Consistent width for numeric columns
    Median_BMI ~ px(60),
    SD_BMI ~ px(60),
    N ~ px(60)
  ) %>%
...
As we can see, our distribution appears to be slightly right-skewed, suggesting the existence of potential non-normality in our data.
...
With respect to `Employment` status, the overall trend suggests that when you are in full-time education, your BMI tends to be lower than if you are permanently unable to work - which is illustrated to result an increase in 1.178 in BMI on average. Both `Looking after home/family` and `Looking for/intending to look for paid work` are not statistically significant in influencing BMI. Also, with respect to lifestyle factors, meeting the recommended `Veg` consumption significantly decreases the BMI - which emphasises the importance of eating vegetables in weight management.
...
library(dplyr)  # Added to ensure select() and everything() are available
anova_df <- as.data.frame(anova_result) %>%
  mutate(Model = c("Stepwise", "Interaction")) %>%
  select(Model, everything())  # Reordering: Model first, then all other columns

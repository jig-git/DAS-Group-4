...
We summarise BMI patterns and visualise distributions to investigate changes over time and differences by age, sex, employment, and diet (specifically fruit and veg intake). This will allow us to inform our Formal Analysis in @sec-FA, by checking **normality** **and variance assumptions**.
...
@tbl-bmi-year tracks changes in the mean, mdedian, standard deviation, and sample size over our time frame.
...
As we can see, @tbl-bmi-year shows that both the mean and median BMI remains between 27 and 28 across 2008-2012. Similarly, standard deviations of remain approximately constant, indicating no clear trend over time but consistent variability.
...
In @fig-bmi-qq, deviation at the tails - particularly at the upper tail - confirms skewness, suggesting a transformation (e.g., log) might be needed for our analysis in @sec-FA.
...
@fig-bmi-diet facets BMI by vegetable and fruit intake across years, which allows us to assess diet effects over time.
...
To ensure that regression assumptions hold, we analyse residuals demonstrated in @fig-residassump below.
